...
Now that we have some code that can pull data from the web and code to generate a visualisation from this data the next step is to make it interactive. This is where Shiny comes in. A great resource for Shiny is [3] which gives a lot of background on how to build up applications. 
...
The following user interface code will build a panel and side-bar page which is a popular structure for a user interface.
...
The server does all the processing to render the content for th ui. Here the server will render a visNetwork consisting of 3 nodes with 2 edges. When you hover over a node this will register on the output box in the UI.
...
To create the app you need to run the following `shinyApp(ui = ui, server = server)`.

```{r, echo=FALSE}
shinyApp(ui = ui, server = server)
```

Many alternative user design options are available you can find more examples of these at [https://shiny.rstudio.com/gallery/widget-gallery.html](https://shiny.rstudio.com/gallery/widget-gallery.html)
...
There are a lot of possible ways to work with network data see the great resource below
